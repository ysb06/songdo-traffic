...
class TrafficDataModule(L.LightningDataModule):
    ...
    def setup(
        ...
        for sensor_name in tqdm(
            all_sensor_list, desc="Creating train-valid datasets..."
        ):
        ...
        if self.valid_split_datetime is not None:
            for dataset in tqdm(
                train_valid_datasets, desc="Splitting to valid datasets..."
            ):
        ...
        if (
            ...
            and len(test_datasets) == 0
        ):
            raise DataInitializationError("No available data for training or testing.")
        ...
        if self.strict_scaler:
            ...
            y_list = []
            for x, y, *_ in tqdm(
                self.train_dataset, desc="Creating strict scaler reference..."
            ):
                x_list.append(x)
