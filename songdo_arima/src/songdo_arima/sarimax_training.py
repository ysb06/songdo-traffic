...
def train_sensor(
    ...
    if not os.path.exists(model_output_path) or force_training:
        ...
        result["period_for_training"] = {"start": str(train_data.index[0]), "end": str(train_data.index[-1])}
        result["period_for_validation"] = {"start": str(valid_data.index[0]), "end": str(valid_data.index[-1])}
