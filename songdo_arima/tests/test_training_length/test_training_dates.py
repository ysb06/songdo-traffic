...
def test_training_short(configs: HyperParams):
    output_root_dir = "./output/arima-01-01-dates"
    ...
    for day_length in range(8, 21):   # 2주가 가장 최적임을 확인했으므로...
        ...
        if mae < best_mae:
            best_mae = mae
            best_mae_week = model_name
            best_mae_week_model = model
            print("Best MAE:", best_mae, "Day:", model_name)

        if rmse < best_rmse:
            best_rmse = rmse
            best_rmse_week = model_name
            best_rmse_week_model = model
            print("Best RMSE:", best_rmse, "Day:", model_name)
    ...
    with open(f"{output_root_dir}/last_model_{target_sensor}_{day_length + 1}.pkl", "wb") as f:
