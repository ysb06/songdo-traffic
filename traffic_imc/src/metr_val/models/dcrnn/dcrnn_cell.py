...
class LayerParams:
    """Helper class for managing layer parameters dynamically."""
...
class DCGRUCell(torch.nn.Module):
    ...
    def _build_sparse_matrix(L):
        """Build sparse matrix from scipy sparse matrix.
        
        Note: Creates on CPU, will be moved to correct device during forward pass.
        """
        L = L.tocoo()
    ...
    def _get_supports_on_device(self, device):
        """Get support matrices on the specified device."""
        return [s.to(device) for s in self._supports]
